...
def get_user_choice(num_files):
    ...
    while True:
        try:
            choice = int(input(f"Enter a number between 1 and {num_files}: "))
...
def analyze_csv(file_name):
    """Perform analysis on the given CSV file."""
...
def main():
    ...
    if not csv_files:
        print("There are no CSV files to analyze.")
        return

    display_options(csv_files)
    user_choice = get_user_choice(len(csv_files))

    selected_file = csv_files[user_choice]
    analyze_csv(selected_file)
